...
import (
	...
	"github.com/sudak-91/telegrambotgo/TelegramAPI/Methods"
...
func (s *SendAnswerTo) Handl(data interface{}) ([]byte, error) {
	...
	uuid, err := base64.RawStdEncoding.DecodeString(server.Util.AnswerCtx[msg.From.ID].QuestionID)
	if err != nil {
	...
	err = s.Question.MarkAsAnswer(uuid)
	if err!=nil{
		Answer.Text ="Внутреняя ошибка"
		log.Printf("SendAnswerTo has SendMessageMethod error: %s",err.Error())
		delete(server.Util.AnswerCtx[msg.From.ID])
