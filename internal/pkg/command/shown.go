...
func (s *Shown) Handl(data interface{}) ([]byte, error) {
	...
	for _, v := range News {
		...
		newsKeyboard.AddButton("Прочесть полностью", fmt.Sprintf("/readmore %s", string(v.NewsID[:])), 0, 0)
		newsKeyboard.AddButton("Отметить как прочитанное", fmt.Sprintf("/markasread %s", string(v.NewsID[:])), 0, 1)
